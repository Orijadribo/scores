...
const Times = () => {
  ...
  return (
    <div className="text-sm md:text-base px-4 md:px-0">
      <div className="grid grid-cols-3 uppercase text-center text-white bg-[#0B6623] rounded-t-md p-2  sticky top-36 md:top-[90px]">
        <p className="col-span-2 md:p-3 flex items-center text-left md:pl-20">
          name
        </p>
        ...
        {sortedGolfData.map((playerData, index) => (
          <div key={index} className="">
            <div className="grid grid-cols-3 hover:bg-[#90EE90]/[0.1] rounded-md p-1">
              <p className=" flex items-center gap-4 col-span-2 p-2 md:pl-20 text-left cursor-pointer hover:underline decoration-[#0B6623]">
                {playerData.PLAYER}
              </p>
